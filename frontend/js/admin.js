...
const API_URL = isLocal
  ...
  : "https://quiz-application-production-c0e1.up.railway.app/api";
...
socket.on("participants-update", (participants) => {
  ...
  if (participantEl) {
    participantEl.innerText = participants.length;
  }
});
...
function showView(view) {
  ...
  if (view === "dashboard") {
    ...
    document
      .querySelector('.nav-item[onclick*="dashboard"]')
...
async function manageQuiz(quizId) {
  ...
  try {
    ...
    if (data.content) {
      document.getElementById("manage-quiz-title").innerText =
        data.content.title;
      document.getElementById("manage-quiz-desc").innerText =
        data.content.description || "";
      document.getElementById("manage-join-code").innerText =
        data.content.joinCode;

      socket.emit("join-quiz", {
        joinCode: data.content.joinCode,
      ...
      startBtn.onclick = async () => {
        if (!data.content.questions || data.content.questions.length === 0) {
          alert(
            "Please add at least one question to the quiz before starting a live room.",
          );
          return;
        }
        ...
        try {
          ...
          window.open(
            `quiz-room.html?admin=true&quizId=${quizId}&code=${data.content.joinCode}`,
            "_blank",
          );
        } catch (e) {
  ...
  ?.addEventListener("submit", async (e) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/create_quiz/question/${currentQuizId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",

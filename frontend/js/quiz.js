...
const BACKEND_URL = isLocal
  ...
  : "https://quiz-application-production-c0e1.up.railway.app";
const socket = io(BACKEND_URL);
const user = JSON.parse(localStorage.getItem("user"));

const urlParams = new URLSearchParams(window.location.search);
const joinCode = urlParams.get("code");
let activeJoinCode = joinCode;
...
function showWaitScreen(type) {
  ...
  if (type === "submitted") {
    ...
    if (message)
      message.innerText = "Great job! Sit tight while the others finish...";
  } else if (type === "timeout") {
    if (icon) icon.innerText = "⏳";
    if (title) title.innerText = "Time's Up!";
    if (message)
      message.innerText =
        "Waiting for the host to publish the next question...";
    ...
    if (message)
      message.innerText =
        "The host has ended the session. Thank you for participating!";
    const loader = document.querySelector("#wait-view .loader");
...
socket.on("new-question", (question) => {
  ...
  document.getElementById("q-marks").innerText =
...
socket.on("final-results", (finalStats) => {
  ...
  if (statsArray.length > 0 && statsArray[0].totalCorrect > 0) {
    ...
    const winners = statsArray.filter(
      (s) =>
    ...
    if (winners.length > 1) {
      if (winnerNameEl)
        winnerNameEl.innerText = winners.map((w) => w.username).join(" & ");
      if (winnerStatsEl)
        winnerStatsEl.innerText = `Tie! Total Correct: ${topScore}`;
    } else {
...
function renderCurrentQuestionLeaderboard(top10) {
  ...
  if (!top10 || top10.length === 0) {
    list.innerHTML =
      '<p style="text-align:center; color:var(--text-muted);">No correct answers yet.</p>';
    return;
  }
...
function endQuiz() {
  if (
    confirm(
      "Are you sure you want to completely close this session? All participant data will be deleted.",
    )
  ) {
    socket.emit("end-quiz", activeJoinCode);
...
function requestFinalResults() {
  ...
  socket.emit("request-final-results", activeJoinCode);
}

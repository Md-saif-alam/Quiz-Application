...
const socketHandler = (io) => {
  io.on("connection", (socket) => {
    ...
    socket.on("publish-question", ({ joinCode, question }) => {
      ...
      io.to(joinCode).emit("new-question", safeQuestion);
    });
    ...
    socket.on("end-quiz", async (joinCode) => {
      if (socket.data.role === "admin") {
        ...
        try {
          ...
          if (quizParticipants[joinCode]) {
            quizParticipants[joinCode].forEach((p) =>
              usernamesToClean.add(p.username),
            );
          ...
          Object.keys(questionLeaderboard).forEach((key) => {
            if (key.startsWith(`${joinCode}_`)) {
              questionLeaderboard[key].forEach((entry) =>
                usernamesToClean.add(entry.username),
              );
    ...
    socket.on("request-final-results", (joinCode) => {
      if (socket.data.role === "admin") {
        const quizUsers = quizParticipants[joinCode]
          ? quizParticipants[joinCode].map((p) => p.username)
          : [];
        const finalStats = {};
